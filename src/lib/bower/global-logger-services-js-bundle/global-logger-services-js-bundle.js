(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["LoggerEntryPoint"] = factory();
	else
		root["LoggerEntryPoint"] = factory();
...
Constants._USE_WINDOW_LOCATION = true;
...
var LoggerUtils = function () {
  ...
  _createClass(LoggerUtils, null, [{
    ...
    value: function GetCurrentConsoleSeverityLevelConfigured() {
      return LoggerUtils.GetLogLevelBySeverity(_Constants.Constants._CONFIGURATIONS["_SEVERITY_CONSOLE"]);
    }
  }, {
    key: "ToStringMessage",
...
var LoggerDispatcher = function () {
	...
	_createClass(LoggerDispatcher, null, [{
		...
		value: function DefaultOutput(usertype, severity, message) {
			severity = severity.toUpperCase();
			if (_Constants.Constants._CONFIGURATIONS["_ENABLE_DEFAULT"] && _LoggerUtils.LoggerUtils.IsAllowedConsoleToLogBySeverity(severity)) {
	...
	_createClass(LoggerRestClient, null, [{
		...
		value: function log(usertype, severity, message) {
			if (_Constants.Constants._CONFIGURATIONS["_ENABLE_AJAX"]) {
				var serverUrl = new URL(_Constants.Constants._CONFIGURATIONS["_SERVER"]);
				var server = new URL(_Constants.Constants._CONFIGURATIONS["_DOMAIN"], serverUrl);
				_axios2.default.post(server, { UserType: usertype, Message: _LoggerUtils.LoggerUtils.ToStringMessage(message), Severity: severity }).catch(function (error) {
	...
	_createClass(LoggerConfigurator, null, [{
		...
		value: function Setup(configuration, callback) {
			...
			if (configuration) {
				...
				console.info('Logger will use default configuration ... ');
			}
